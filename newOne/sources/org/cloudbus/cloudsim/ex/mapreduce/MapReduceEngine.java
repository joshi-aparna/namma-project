...
public class MapReduceEngine extends DatacenterBroker {
	...
	for (Request request : requests.requests) {
		if(request.job.mapTasks!=null){
			submitCloudletList(request.job.mapTasks);
			 // Set the MapReduce Engine referance to all map tasks.
	...
	if (getDatacenterCharacteristicsList().size() == getDatacenterIdsList().size()) {
	    ...
	    for (Request request : requests.requests){
	    	System.out.println("-----------in mapreduce engine processresource characteristics: request being sent="+request);
	...
	//int datacenterId = Integer.parseInt(MultiSimulation.getCurrentID());
	int vmId = data[1];
	int result = data[2];
	...
	if (result == CloudSimTags.TRUE) {
	    ...
	    System.out.println("mapreduceengine:getVmsToDatacenterMap put:"+vmId+":"+datacenterId);
	...
	//vms.csv logging
	CustomLog.redirectToFile("output/logs/vms"+id+".csv", true);
	for (Request request : requests.requests) {
	...
	//tasks.csv logging
		CustomLog.redirectToFile("output/logs/tasks"+id+".csv", true);
	...
	CustomLog.redirectToFile("output/logs/requests"+id+".csv", true);
